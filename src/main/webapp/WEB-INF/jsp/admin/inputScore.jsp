				...
				<div class="navbar-default sidebar" role="navigation">
					<div class="sidebar-nav navbar-collapse">
						<ul class="nav" id="side-menu">
							...
							<li><a href="${pageContext.request.contextPath}/AinputScore" class="active"><i class="fa fa-dashboard fa-fw"></i> 录入成绩</a></li>
			...
			<div id="page-wrapper">
				...
				<div class="panel panel-default">
					<div class="panel-body">
						<form class="form-inline" action="${pageContext.request.contextPath }/AinputScore" method="post">
				...
				<form action="${pageContext.request.contextPath}/admin/inputScore" method="post" onsubmit="return check()">
								...
								<table class="table table-bordered table-striped ">
									...
									<tbody class="text-center">
										<c:forEach items="${studentList}" var="item" varStatus="status">
											<input type="hidden"  value="${item.id}" name="examInfoList[${status.index}].id"/>
											<input type="hidden"  value="${item.kemu}" name="examInfoList[${status.index}].kemu"/>
											<tr>
												<td>${item.school}</td>
												<td>${item.zhuanye}</td>
												<td>${item.xuehao}</td>
												...
												<td >${item.kemu}</td>
												<td class="td">
													<input type="text"  name="examInfoList[${status.index}].score" style="width: 45px;"/><label>&nbsp;&nbsp;分</label>
												</td>
											</tr>
										</c:forEach>
											<tr>
												...
												<td>
													<c:if test="${studentList!=null and studentList.size()>0}">
														<button class="btn-sm btn-primary" type="submit"  style="text-align: right;">提交成绩</button>
													</c:if>
